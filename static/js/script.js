...
state: {
    ...
    config: {
        ...
        shell: 'Random',
        size: IS_DESKTOP ? '4' : IS_HEADER ? '1.2' : '2',
        autoLaunch: false,
        finale: false,
...
setOptionsForSelect(appNodes.scaleFactor, [0.5, 0.62, 0.75, 0.9, 1.0, 1.5, 2.0].map(value => ({
...
const promise = fetch(fileURL).then(checkStatus).then(response => response.arrayBuffer()).then(data => new Promise(resolve => 
{
